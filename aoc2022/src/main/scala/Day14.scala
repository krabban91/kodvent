...
object Day14 extends App with AoCPart1Test with AoCPart2Test {
  ...
  override def part2(strings: Seq[String]): Long = {
    ...
    val lowestY = paths.map(_.map(_._2).max).max + 2
    val sandSource = (500,0)
    logMap(sandMap)

    var highestSand = sandSource._2
    while (!sandMap.keySet.contains(sandSource)){
      var sand = sandSource
      var moves = true
      while (moves) {
        ...
        if(sandMap.keySet.contains(below)){
          ...
          if(!sandMap.keySet.contains(belowLeft)){
            ...
            moves = false
            //logMap(sandMap)
            highestSand = math.max(sand._2, highestSand)
          }
        } else {
          //below
          sand = below
