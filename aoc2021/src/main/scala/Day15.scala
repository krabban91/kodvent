...
object Day15 extends App with AoCPart1Test with AoCPart2Test {
  ...
  override def part2(strings: Seq[String]): Long = {
    val vv: Seq[Seq[Long]] = strings.map(s=>s.chars().mapToObj(_.toChar.asDigit.toLong).collect(Collectors.toList[Long]).asScala.toSeq)
    val tMaps = vv.zipWithIndex.flatMap(t0 => t0._1.zipWithIndex.map(t=> ( (t0._2, t._2),t._1))).toMap
    val maps = (0 until 5).flatMap(x => (0 until 5).flatMap(y =>tMaps.flatMap(t => Seq(((t._1._1 + vv.head.size*x, t._1._2 + vv.size * y), if((t._2 + x+y)>9) (t._2 + x+y)%10 + 1 else (t._2 + x+y)),((t._1._1+ vv.head.size*x, t._1._2 + vv.size*y), if((t._2 + x+y)>9) (t._2 + x+y)%10 + 1 else (t._2 + x+y)))))).toMap
