...
object Day22 extends App with AoCPart1Test with AoCPart2Test {
  ...
  override def part1(strings: Seq[String]): Long = {
    val tower: Map[Day22.Brick, Seq[Day22.Brick]] = parse(strings)
    tower.count { case (b, bs) => bs.forall(v => tower.exists(o => o._1 != b && o._2.contains(v))) }
  }


  override def part2(strings: Seq[String]): Long = {
    val tower: Map[Day22.Brick, Seq[Day22.Brick]] = parse(strings)
    val topples = tower.map { case (b, _) =>
      val supportCopy = mutable.HashMap[Day22.Brick, Seq[Day22.Brick]]()
      supportCopy.addAll(tower)
  ...
  case class Brick(from: (Long, Long, Long), to: (Long, Long, Long)) {
    ...
    private def xOverLap(other: Brick): Boolean = {
    ...
    private def yOverLap(other: Brick): Boolean = {
